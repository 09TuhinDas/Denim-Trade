...
ALLOW_SHORT = True  # ✅ Toggle for short support

def simulate_trade(entry_price, future_prices, direction="long", profit=PROFIT_TARGET, hold_days=HOLD_DAYS):
    for i, price in enumerate(future_prices[1:hold_days+1]):
        sell_price = price * 0.998  # 0.2% slippage
        if direction == "long":
            ret = (sell_price - entry_price) / entry_price
        else:
            ret = (entry_price - sell_price) / entry_price
    ...
    if direction == "long":
        ...
        final_ret = (entry_price - final_sell) / entry_price
...
def backtest_ticker(ticker, model, window=14):
    ...
    for i in range(len(df) - window - HOLD_DAYS):
        ...
        if label == 2 and not ALLOW_SHORT:
            continue

        direction = "long" if label == 1 else "short"

        raw_entry_price = test['close'].values[0]
        entry_price = raw_entry_price * 1.001
        future_prices = df['close'].iloc[i+window:i+window+HOLD_DAYS+1].values

        ret, days_held, exit_reason = simulate_trade(entry_price, future_prices, direction=direction)
        ...
        print(f"📊 {ticker} | {direction.upper()} | {exit_reason} | Conf: {confidence:.2f} | Ret: {ret:.2%} | PnL: ₹{trade_pnl:.2f}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        df = pd.read_csv("data/raw/nse_equity_list.csv")
